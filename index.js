window.addEventListener('load', function() {
    ...
    for (var i = 0; i < spans.length; i++) {
        spans[i].index = i; //给每一份span添加代表自生是第几个的属性。
    ...
    next.addEventListener('click', function() {
        for (var j = 0; j < spans.length; j++) {
            if (spans[j].className == "current") {
                // 先找出当前在哪个span，找到了之后 令当前的banner消失，小圆圈变回淡色，然后给下一个小圆圈上色，让下一个banner出现。
                spans[j].className = "";
                banners[j].style.opacity = "0";
                j++;
    ...
    prev.addEventListener('click', function() {
        for (var j = 0; j < spans.length; j++) {
            if (spans[j].className == "current") {
                // 先找出当前在哪个spans，找到了之后 令当前的banner消失，小圆圈变回淡色，然后给下一个小圆圈上色，让下一个banner出现。
                spans[j].className = "";
                banners[j].style.opacity = "0";
                j--;

                if (j < 0) { //如果正好是最后一个banner，则下一个banner是第一个。j=0；
                    j = 4;
                }
                spans[j].className = "current";
                banners[j].style.opacity = "1";
